...
> 📀 一个为异地无头树莓派设计的全自动系统刷写脚本

在**无法物理接触树莓派、没有桌面环境**的情况下，安全地将 Raspberry Pi OS 烧录到 USB SSD / NVMe 等存储设备，并自动完成 **SSH、用户密码、WiFi** 等无头预配置。

✅ 全程交互式引导 <br>
✅ 自动获取最新镜像列表（仅支持64位）<br>
✅ 自动安装 `rpi-imager` 依赖 <br>
✅ 多重安全保护：禁止覆盖当前系统盘 <br>
✅ 支持 SD 卡、eMMC、NVMe 等设备 <br>
✅ 镜像缓存复用，省时省流量 <br>
